...
ncbi = NCBITaxa
def taxa_cumulation(reads_dictionary, basic_ranks=['superkingdom',
                        'phylum',
                        'class',
                        'order',
                        'family',
    ...
    for taxid_key in taxid_key_list:
        ordered_lineage = transform_lineage(taxid_key, basic_ranks)
        ...
        if taxid_key != ordered_lineage[0]: ## if the current taxid(taxid_key) is not the lowest rank (not strain/subsps i.e. anything beyond species level)
            reads_dictionary = update_dictfile(ordered_lineage[0], taxid_key, reads_dictionary, basic_ranks)
    ...
    ranks = update_ranks(reads_dictionary)
    ...
    for i, r in enumerate(flipped_ranks[:-1]): ## this reverses it to start from sps rank, so sps read count will be added to genus and genus to family and so on. 
        ...
        for taxid in ranks[r]:
            parent_taxid = find_parent_taxid(taxid, parent_rank, flipped_ranks)
            reads_dictionary[parent_taxid] += reads_dictionary[taxid]

    return reads_dictionary

def transform_lineage(taxanomy_id, basic_ranks):
    """
...
def update_ranks(reads_dictionary):
    """
...
def update_dictfile(taxanomy_id, dictionary_key, reads_dictionary, basic_ranks):
    """
...
def find_parent_taxid(taxid, parent_rank, taxanomy_ranks):
    """
...
def turn_reads_to_clade_relative_abundance(reads_dictionary, basic_ranks=['superkingdom',
                        'phylum',
                        'class',
                        'order',
                        'family',
    ...
    """
    ranks = update_ranks(reads_dictionary)
    clade_read_counts = get_clade_read_counts(reads_dictionary, basic_ranks, ranks)
...
def get_clade_read_counts(reads_dictionary, basic_ranks, ranks):
    """
...
def select_rank(reads_dictionary, desired_ranks):
    """
