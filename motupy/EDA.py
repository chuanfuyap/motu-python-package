...
def filter_otu_coverage(dataframe, otu_coverage=0.0025):
    """
...
def clean_artifact(dataframe, artifact_threshold=5):
    """
...
def aitchison_distance_matrix(df):
    """
    ...
    for col_1 in col_names:
        for col_2 in col_names:
            ...
            scored = add_to_dict(scored, col_1, col_2,  distance_score)
            scored = add_to_dict(scored, col_2, col_1,  distance_score)
            
    return pd.DataFrame(scored)

def add_to_dict(the_dict, column_name, index_name, value):
    """
...
def generate_aitchison_pcoa(df):
    """
    ...
    """
    dist_matrix = aitchison_distance_matrix(df)
